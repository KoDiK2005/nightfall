...
func _run() -> void:
	...
	# 8 -- сундук/шкафчик/ящик/стол) не должен стоять так близко к горловине
	# дверного проёма, что там физически не протиснется капсула игрока,
	# иначе он перегораживает единственный путь в дверь (жалоба "некоторые
	# вещи не дают пройти в двери"). Раньше порог был единой константой 0.6
	# для ЛЮБОГО пропа -- не учитывал ни реальный габарит коллайдера (у стола
	# полудиагональ ~0.51, у ящика ~0.35 -- разница почти вдвое), ни то, что
	# стол ставится со случайным поворотом (см. level_gen.gd::_spawn_desk),
	# так что его угол может вылезти в проём даже когда центр формально
	# дальше. Теперь порог считается по факту: полудиагональ коллайдера в
	# плане (XZ) + радиус капсулы игрока (см. main.tscn::CapsuleShape3D_player)
	# + небольшой запас на диагональный подход, а не одна и та же цифра для
	# всех габаритов. Перебираем больше свежих этажей (было 6), чтобы поймать
	# редкий неудачный расклад, а не только текущую генерацию.
		...
		for d in lg.doors:
			...
			for body in lg.props_root.get_children():
				if not (body is StaticBody3D and body.collision_layer == 8):
					continue
				var half_diag: float = 0.25   # запасной вариант, если формы нет (не должно случаться)
				for c in body.get_children():
					if c is CollisionShape3D and c.shape is BoxShape3D:
						var sz: Vector3 = c.shape.size
						half_diag = Vector2(sz.x, sz.z).length() / 2.0
						break
				var required: float = half_diag + PLAYER_RADIUS + 0.08
				var bp := Vector2(body.position.x, body.position.z)
				if bp.distance_to(mouth) < required or bp.distance_to(d.pos) < required:
					doorway_clear = false
	...
	# явный сброс к канону -- раньше проверки "стартовый запас Х" ниже
	# полагались на то, что счётчики со старта этажа не изменились, но пока
	# тест телепортировал игрока по сундукам (см. шаг 2 выше), _collect_pickups
	# (вызывается каждый кадр безусловно) мог по пути подобрать спичку/камень/
	# обмотки, если те случайно заспавнились рядом с каким-то сундуком --
	# наблюдаемая с этим тестом флаки-нестабильность ("стартовый запас
	# спичек -- 2" изредка падал). Секции 4-6 ниже тестируют МЕХАНИКУ
	# предметов (трата счётчика при использовании), а не то, что нашлось по
	# пути к сундукам -- сброс к канону убирает эту случайность целиком.
	items.match_count = 2
	items.rock_count = 2
	items.flare_count = 1
	items.wrap_count = 1
	...
	# позиции -- на lg.exit_pos (гарантированно открытая клетка на любом
	# сгенерированном этаже, тем же приёмом уже пользуется тест кейджа/крови
	# чуть выше), а не фиксированные (2,0): та клетка могла оказаться стеной
	# на конкретной случайной карте, и _has_los()'ный raycast между игроком
	# и монстром на нулевой дистанции (обе точки совпадали) вёл себя
	# непредсказуемо -- отсюда флаки именно этого теста, не игровой код.
	var pmon: Node = player.monster
	pmon.mon_type = pmon.MonType.STALKER
	pmon.state = pmon.State.SEARCH
	pmon.pause_timer = 1.0
	player.position = Vector3(lg.exit_pos.x, 0.9, lg.exit_pos.y)
	pmon.position = Vector3(lg.exit_pos.x + 0.3, 0.1, lg.exit_pos.y)
	player.hidden = false
	pmon._sense(0.016)
