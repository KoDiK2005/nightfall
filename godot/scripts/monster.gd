...
func _sense(delta: float) -> void:
	...
	# обмотки (items.gd::_wrap_feet) -- временный контрприём против слуха:
	# приглушают собственные шаги на время, пока горят.
	# Красться (player.gd::crouched) -- ПОСТОЯННЫЙ бесплатный контрприём той
	# же природы, но слабее разово и ценой скорости: множители слуха
	# перемножаются, так что присед+обмотки дают почти полную тишину
	# (0.35*0.45 ~= 0.16 радиуса слуха).
	var muffle: float = 0.35 if player.muffled else 1.0
	if player.crouched:
		muffle *= 0.45
	# "низкий силуэт" в присяде -- глазастый Сталкер (и любой видящий) замечает
	# чуть позже, зрение урезано на ~25%. Слухача это не касается (он слеп),
	...
	if mon_type == MonType.LISTENER:
		...
		var see_range: float = SEE_RANGE * (1.0 + w * 0.3) * see_mult * (1.9 if player.lit_by_match else 1.0)
