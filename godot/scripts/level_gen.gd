...
## реальная мировая высота верхней грани пола -- НЕ 0, вопреки комментарию
## у FLOOR_CLEARANCE ниже и предположению почти всех спавнеров в этом
## файле. FloorGridMap кладёт тайл на слой 0, а GridMap.map_to_local()
## возвращает ЦЕНТР клетки (cell_center_y по умолчанию true), так что даже
## с mesh_transform пола y=-0.05 верх плитки выходит на 0.5, не на 0
## (замерено напрямую: floor_map.get_cell_item + get_item_mesh_transform +
## Mesh.get_aabb()). Из-за этого сундук ("сундук под землёй, а ключ над
## ним") был не виден целиком, ноги монстра тонули по щиколотку, и т.д.
## Все объекты под props_root (кроме факелов -- torch_root отдельный
## узел, height 1.3 у них подобран вручную по скриншотам и не завязан на
## эту формулу) сдвинуты на GROUND_Y разом через props_root.position.y
## ниже в _build_level(), а не поодиночке в каждом спавнере -- так гарантированно
## не пропустить ни один. Игрок/монстр/фантом -- отдельные узлы вне
## props_root, им GROUND_Y добавлен в их собственных местах спавна.
const GROUND_Y := 0.5
...
const NUM_LOCKERS := 3   # было 5 -- на старте комнаты в глаза бросалось сразу несколько шкафов подряд, перебор
## зазор между низом декоративного меша и полом. Раньше здесь было написано
## "y=0, см. tiles.tres -- верхняя грань пола ровно на нуле" -- неверно, см.
## GROUND_Y выше: реальная верхняя грань на 0.5. Щебень/подобранные камни/
...
var crate_spots: Array = []   # Vector2 позиций ВСЕХ ящиков (не только supply) -- для укрытия при приседе, см. player.gd::near_cover
...
func _ready() -> void:
	randomize()
	# см. GROUND_Y выше -- единая точка фикса вместо правки каждого
	# спавнера по отдельности: всё, что кладётся через props_root.add_child
	# (сундуки, шкафчики, ящики, алтарь, столы, кости, клетки, лужи крови,
	# паутина, щебень, подбираемые предметы, дверные рамы) наследует этот
	# сдвиг автоматически, ничего в самих спавнерах менять не пришлось.
	props_root.position.y = GROUND_Y
...
func _spawn_monster() -> void:
	...
	add_child(monster)
	# monster -- ребёнок LevelGen (не props_root), GROUND_Y сюда не приходит
	# автоматически -- добавляем явно, тот же баг, что был у сундука: без
	# этого ноги монстра тонут в полу по щиколотку (замерено: мировая Y
	# рига ~0.15 при реальном верхе пола 0.5).
	monster.position = Vector3(best.x + 0.5, GROUND_Y + 0.1, best.y + 0.5)
	...
	if OS.get_environment("NIGHTFALL_SHOWMON") != "":
		...
		monster.position.y = GROUND_Y + 0.1
		monster.frozen = true
...
func _spawn_phantom() -> void:
	...
	add_child(_phantom)
	_phantom.position = Vector3(pos.x, GROUND_Y + 0.1, pos.z)   # см. GROUND_Y -- та же поправка, что и у настоящего монстра
...
func _build_door_frame(cell: Vector2i, dir: Vector2i, wood_mat: StandardMaterial3D, wall_mat: StandardMaterial3D) -> void:
	...
	var jamb_half := 0.48   # почти во всю клетку -- проём шире, чем был (0.46)
	# ВЫСОТА ПРОЁМА. Монстр ВЫШЕ игрока -- его макушка на ~2.05 (замерено),
	# и при верхе проёма 1.78 голова монстра протыкала кладку над дверью
	# ("монстр не помещается"). Поднимаем верх проёма выше макушки монстра
	# (2.2 -- запас ~0.15), места хватает: реальный потолок на WALL_H+0.5=2.5
	# (см. ниже). Игроку (камера 1.5) это только добавляет запаса над головой,
	# он и не бился. Полотно во всю высоту проёма.
	var leaf_h: float = 2.15
	var leaf_top: float = leaf_h + 0.05   # = 2.2
...
func _place_lockers(exit_room_idx: int) -> void:
	...
	for i in candidates:
		...
		for c in cells:
			...
			_add_beacon(shell, Color(0.65, 0.7, 0.78), 0.3, 1.7, 0.5)
			# раньше шкафчик был цельной коробкой -- E мгновенно телепортировал
			# камеру поверх неё с плоским шейдер-оверлеем ("вентиляция"), без
			# намёка на реальный объём. Теперь это настоящая полая коробка
			# (спинка+борта+крышка, дно -- сам пол) с дверцей на петле
			# (_open_locker_door/_close_locker_door), и дверца -- набор
			# горизонтальных ламелей со щелями между ними: даже прикрытая,
			# сквозь неё физически видно комнату, потому что камера теперь
			# правда стоит внутри объёма шкафа, а не просто получает шейдер.
			var hx: float = locker_size.x / 2.0
			var hy: float = locker_size.y / 2.0
			var hz: float = locker_size.z / 2.0
			var front_n := Vector3(-wall_dir.x, 0, -wall_dir.y)
			var right_n := Vector3(0, 0, 1) if wall_dir.x != 0 else Vector3(1, 0, 0)
			var thick_x: bool = wall_dir.x != 0   # передняя грань смотрит вдоль X

			var back := MeshInstance3D.new()
			back.mesh = BoxMesh.new()
			back.mesh.size = Vector3(locker_size.x, locker_size.y, 0.05) if thick_x \
				else Vector3(0.05, locker_size.y, locker_size.z)
			back.material_override = locker_mat
			back.position = -front_n * (hz - 0.025)
			shell.add_child(back)
			for side_sign in [-1.0, 1.0]:
				var panel := MeshInstance3D.new()
				panel.mesh = BoxMesh.new()
				panel.mesh.size = Vector3(0.05, locker_size.y, locker_size.z) if thick_x \
					else Vector3(locker_size.x, locker_size.y, 0.05)
				panel.material_override = locker_mat
				panel.position = right_n * side_sign * (hx - 0.025)
				shell.add_child(panel)
			var top := MeshInstance3D.new()
			top.mesh = BoxMesh.new()
			top.mesh.size = Vector3(locker_size.x, 0.05, locker_size.z)
			top.material_override = locker_mat
			top.position = Vector3(0, hy - 0.025, 0)
			shell.add_child(top)

			# дверца -- пивот-петля на "правом" крае передней грани (тот же
			# приём, что и у сундука/двери выхода: пивот стоит у петли,
			# полотно -- его ребёнок со смещением вдоль ширины). Раньше
			# полотно целиком состояло из 4 ламелей во всю высоту -- со
			# стороны это читалось как решётка/забор, а не как дверь. Теперь
			# дверь В ОСНОВНОМ сплошная (два цельных панельных куска), и
			# только небольшая вентиляционная решётка ближе к верху -- как у
			# настоящего металлического шкафчика, а не целиком из щелей.
			var door_pivot := Node3D.new()
			door_pivot.position = front_n * hz + right_n * hx
			shell.add_child(door_pivot)
			var panel_w: float = hx * 2.0 - 0.04
			var door_bottom: float = -hy + 0.05
			var door_top: float = hy - 0.05
			var vent_center: float = 0.32
			var vent_h: float = 0.3
			var vent_top: float = vent_center + vent_h / 2.0
			var vent_bottom: float = vent_center - vent_h / 2.0

			var lower_h: float = vent_bottom - door_bottom
			var lower := MeshInstance3D.new()
			lower.mesh = BoxMesh.new()
			lower.mesh.size = Vector3(0.04, lower_h, panel_w) if thick_x \
				else Vector3(panel_w, lower_h, 0.04)
			lower.material_override = locker_mat
			lower.position = -right_n * hx + Vector3(0, door_bottom + lower_h / 2.0, 0)
			door_pivot.add_child(lower)

			var upper_h: float = door_top - vent_top
			var upper := MeshInstance3D.new()
			upper.mesh = BoxMesh.new()
			upper.mesh.size = Vector3(0.04, upper_h, panel_w) if thick_x \
				else Vector3(panel_w, upper_h, 0.04)
			upper.material_override = locker_mat
			upper.position = -right_n * hx + Vector3(0, vent_top + upper_h / 2.0, 0)
			door_pivot.add_child(upper)

			# вентиляционная решётка -- 3 узкие ламели с редкими щелями,
			# ровно в пределах vent_bottom..vent_top, не во всю дверь.
			var gap: float = 0.045
			var slat_h: float = (vent_h - gap * 2.0) / 3.0
			var vy0: float = vent_bottom + slat_h / 2.0
			for si in range(3):
				var slat := MeshInstance3D.new()
				slat.mesh = BoxMesh.new()
				slat.mesh.size = Vector3(0.035, slat_h, panel_w) if thick_x \
					else Vector3(panel_w, slat_h, 0.035)
				slat.material_override = locker_mat
				slat.position = -right_n * hx + Vector3(0, vy0 + si * (slat_h + gap), 0)
				door_pivot.add_child(slat)
			...
			handle.position = -right_n * (hx * 2.0 - 0.08) + Vector3(0, vent_bottom - 0.18, 0)
			door_pivot.add_child(handle)

			# как и у сундука -- своя коллизия (см. _add_prop_collision), но
			# при входе в шкафчик игрок анимированно въезжает на interior_pos
			# (см. player.gd::_enter_locker): столкновение с СОБСТВЕННЫМ
			...
			var locker_root := _add_prop_collision(shell, Vector3(wx, 0.9, wz), locker_size)
			player.lockers.append({
				"pos": Vector2(wx, wz),
				"root": locker_root,
				"door_pivot": door_pivot,
				"is_open": false,
				"interior_pos": Vector3(wx, GROUND_Y + 0.9, wz),
				"front": front_n,
			})

## распахнуть дверцу конкретного шкафчика (вызывается из player.gd на входе
## и выходе, тем же tween-приёмом, что и у сундука/двери выхода). l -- запись
## из player.lockers (Dictionary, передаётся по ссылке, так что is_open
## меняется прямо в массиве).
func open_locker_door(l: Dictionary) -> void:
	if l.is_open:
		return
	l.is_open = true
	var tw := create_tween()
	tw.tween_property(l.door_pivot, "rotation:y", -deg_to_rad(110.0), 0.35) \
		.set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_OUT)

func close_locker_door(l: Dictionary) -> void:
	if not l.is_open:
		return
	l.is_open = false
	var tw := create_tween()
	tw.tween_property(l.door_pivot, "rotation:y", 0.0, 0.4) \
		.set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN)
...
func _spawn_player() -> void:
	...
	# 0.9 -- высота центра капсулы НАД ПОЛОМ (гравитации нет, ставим руками),
	# не абсолютная мировая Y: реальный пол на GROUND_Y=0.5, не на 0 (см.
	# константу выше), так что фактическая мировая высота -- GROUND_Y+0.9.
	# Глаз камеры (+0.6) оказывается на ~2.0м над полом -- нормальный
	# взрослый рост, а не пригнувшийся вид, каким он был при "полу на 0".
	player.position = Vector3(cx, GROUND_Y + 0.8, cy)
	...
	if OS.get_environment("NIGHTFALL_SHOWEXIT") != "":
		keys_left = 0
		player.position = Vector3(exit_pos.x, GROUND_Y + 0.8, exit_pos.y - 2.5)
		player.look_at(Vector3(exit_pos.x, GROUND_Y + 0.8, exit_pos.y), Vector3.UP)
		hud_changed.emit()
		_open_exit_door()
	...
	if OS.get_environment("NIGHTFALL_SHOWDOOR") != "" and not doors.is_empty():
		...
		player.position = Vector3(dp.x, GROUND_Y + 0.8, dp.y)
		player.look_at(Vector3(d0.pos.x, GROUND_Y + 0.8, d0.pos.y), Vector3.UP)
	...
	if OS.get_environment("NIGHTFALL_SHOWCRATE") != "" and not supply_crates.is_empty():
		...
		player.position = Vector3(sc.pos.x - 0.9, GROUND_Y + 0.8, sc.pos.y)
		player.look_at(Vector3(sc.pos.x, GROUND_Y + 0.8, sc.pos.y), Vector3.UP)
	...
	# не обходя весь этаж в поисках подходящей комнаты. ds.y хранится
	# props_root-локально (та же величина, что и позиция самой записки в
	# notes.gd -- она тоже добавлена под props_root, поэтому её трогать не
	# пришлось), а игрок -- отдельный узел вне props_root, поэтому здесь
	# GROUND_Y нужно добавить явно, иначе игрок встанет на 0.5 ниже стола.
	if OS.get_environment("NIGHTFALL_SHOWDESK") != "" and not desk_note_spots.is_empty():
		var ds = desk_note_spots[0]
		player.position = Vector3(ds.pos.x, GROUND_Y + ds.y + 1.1, ds.pos.y + 0.01)
		player.look_at(Vector3(ds.pos.x, GROUND_Y + ds.y, ds.pos.y), Vector3.UP)
	...
	if OS.get_environment("NIGHTFALL_SHOWALTAR") != "" and not altar.is_empty():
		player.position = Vector3(altar.pos.x - 0.8, GROUND_Y + 0.8, altar.pos.y)
		player.look_at(Vector3(altar.pos.x, GROUND_Y + 0.7, altar.pos.y), Vector3.UP)
	...
	if OS.get_environment("NIGHTFALL_SHOWCELL") != "" and not cage_spots.is_empty():
		...
		player.position = Vector3(cs.x, GROUND_Y + 0.8, cs.y - 1.2)
		player.look_at(Vector3(cs.x, GROUND_Y + 0.8, cs.y), Vector3.UP)

	# dev-хук NIGHTFALL_SHOWLOCKER: встать перед первым шкафчиком этажа лицом
	# к дверце -- для скриншотов геометрии/анимации входа без обхода этажа.
	if OS.get_environment("NIGHTFALL_SHOWLOCKER") != "" and not player.lockers.is_empty():
		var lk: Dictionary = player.lockers[0]
		var stand: Vector3 = lk.interior_pos + lk.front * 1.4
		player.position = Vector3(stand.x, GROUND_Y + 0.8, stand.z)
		player.look_at(Vector3(lk.pos.x, GROUND_Y + 0.8, lk.pos.y), Vector3.UP)
