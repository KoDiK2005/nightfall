...
func _spawn_player() -> void:
	...
	player.position = Vector3(cx, GROUND_Y + 0.9, cy)
	...
	if OS.get_environment("NIGHTFALL_SHOWEXIT") != "":
		keys_left = 0
		player.position = Vector3(exit_pos.x, GROUND_Y + 0.9, exit_pos.y - 2.5)
		player.look_at(Vector3(exit_pos.x, GROUND_Y + 0.9, exit_pos.y), Vector3.UP)
		hud_changed.emit()
		_open_exit_door()
	...
	if OS.get_environment("NIGHTFALL_SHOWDOOR") != "" and not doors.is_empty():
		...
		player.position = Vector3(dp.x, GROUND_Y + 0.9, dp.y)
		player.look_at(Vector3(d0.pos.x, GROUND_Y + 0.9, d0.pos.y), Vector3.UP)
	...
	if OS.get_environment("NIGHTFALL_SHOWCRATE") != "" and not supply_crates.is_empty():
		...
		player.position = Vector3(sc.pos.x - 0.9, GROUND_Y + 0.9, sc.pos.y)
		player.look_at(Vector3(sc.pos.x, GROUND_Y + 0.9, sc.pos.y), Vector3.UP)
	...
	if OS.get_environment("NIGHTFALL_SHOWALTAR") != "" and not altar.is_empty():
		player.position = Vector3(altar.pos.x - 0.8, GROUND_Y + 0.9, altar.pos.y)
	...
	if OS.get_environment("NIGHTFALL_SHOWCELL") != "" and not cage_spots.is_empty():
		...
		player.position = Vector3(cs.x, GROUND_Y + 0.9, cs.y - 1.2)
		player.look_at(Vector3(cs.x, GROUND_Y + 0.9, cs.y), Vector3.UP)
